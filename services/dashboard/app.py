...
st.set_page_config(page_title="NOAH Unified Commerce Dashboard", page_icon="📊", layout="wide")
...
.ok {background:#dcfce7; color:#166534;} .warn {background:#fef3c7; color:#92400e;} .info {background:#dbeafe; color:#1d4ed8;} .bad {background:#fee2e2; color:#b91c1c;}
...
def fetch_report(page:int, limit:int):
    r = requests.get(f"{GATEWAY_URL}/report/api/report", params={"page":page,"limit":limit}, headers=auth_headers(), timeout=20)
    r.raise_for_status()
    return r.json()
...
def fetch_queue():
    try:
        r = requests.get(f"{RABBITMQ_MGMT_URL}/api/queues/%2F/order_queue", auth=(RABBITMQ_USER,RABBITMQ_PASS), timeout=10)
        if r.status_code == 200:
            d = r.json()
            return {"messages": d.get("messages",0), "ready": d.get("messages_ready",0), "unacked": d.get("messages_unacknowledged",0), "consumers": d.get("consumers",0), "state": d.get("state","unknown")}
    except Exception:
        pass
    return {"messages":0,"ready":0,"unacked":0,"consumers":0,"state":"unavailable"}

def send_order(user_id:int, product_id:int, quantity:int):
    return requests.post(f"{GATEWAY_URL}/orders/api/orders", json={"user_id":user_id,"product_id":product_id,"quantity":quantity}, headers={**auth_headers(),"Content-Type":"application/json"}, timeout=20)

def kpi_card(title, value, sub=""):
    st.markdown(f'<div class="kpi-card"><div class="kpi-title">{title}</div><div class="kpi-value">{value}</div><div class="kpi-sub">{sub}</div></div>', unsafe_allow_html=True)

def pill(text, cls):
    st.markdown(f'<span class="status-pill {cls}">{text}</span>', unsafe_allow_html=True)
...
report_error = None
try:
    report = fetch_report(page, limit)
...
c = st.columns(5)
with c[0]: pill("MySQL / Orders", "ok" if not report_error else "bad")
with c[1]: pill("PostgreSQL / Finance", "ok" if not report_error else "bad")
with c[2]: pill(f"RabbitMQ / {queue['state']}", "ok" if queue["state"] != "unavailable" else "warn")
with c[3]: pill("Kong Protected", "info")
with c[4]: pill("API Key Enabled", "info")
...
with k1: kpi_card("Total Orders", str(pagination.get("total_orders",0)), "Tổng số đơn trong hệ thống")
with k2: kpi_card("Rows on Page", str(summary.get("orders_in_this_page",0)), f"Trang {pagination.get('page',1)}")
with k3: kpi_card("Synced Orders", str(summary.get("synced_orders_in_page",0)), "Đã đối soát sang Finance")
with k4: kpi_card("Pending Orders", str(summary.get("pending_orders_in_page",0)), "Đang chờ worker xử lý")
with k5: kpi_card("Page Revenue", f"{summary.get('page_total_revenue',0):,.0f}", "Doanh thu trang hiện tại")
with k6: kpi_card("Queue Ready", str(queue.get("ready",0)), f"Unacked: {queue.get('unacked',0)}")
...
with l:
    ...
    if not top_df.empty:
        fig = px.bar(top_df, x="user_id", y="total_revenue", text="total_revenue", labels={"user_id":"User ID","total_revenue":"Revenue"})
...
with r:
    ...
    sync_df = pd.DataFrame([{"Status":"SYNCED","Count":summary.get("synced_orders_in_page",0)},{"Status":"PENDING","Count":summary.get("pending_orders_in_page",0)}])
    ...
    st.markdown(f"<div class='small-note'>Queue snapshot — messages: {queue.get('messages',0)}, ready: {queue.get('ready',0)}, unacked: {queue.get('unacked',0)}, consumers: {queue.get('consumers',0)}</div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
st.markdown("---")
st.caption(f"Last rendered at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Gateway: {GATEWAY_URL} | Page {pagination.get('page',1)}/{pagination.get('total_pages',1)}")
